...
class GrossProfitGenerator:
	...
	def process(self):
		...
		for entry in self.stock_entries:
			...
			if not item_code:
				continue
			entry["buying_amount"] = entry.get("basic_amount", 0.0)
			if item_code not in self.grouped:
				self.grouped[item_code] = [{
					"qty": entry.get("qty", 0),
					"buying_amount": entry.get("basic_amount", 0.0),
					"item_name": entry.get("item_name", 0.0),
					"item_code": item_code,
					"description": entry.get("description", 0.0),
					# "basic_rate": entry.get("basic_rate", 0.0)
				}]
			else:
				for row in self.grouped[item_code]:
					row["qty"] += entry.get("qty", 0)
					row["buying_amount"] += entry.get("basic_amount", 0.0)
					# row["basic_rate"] = entry.get("basic_rate", 0.0)

		# for item_code, rows in self.grouped.items():
		# 	for row in rows:
		# 		if flt(row.get("qty", 0)):
		# 			# row.buying_rate = flt(row.buying_amount / flt(row.qty), self.float_precision)
		# 			row["buying_rate"] = flt(row.get("buying_amount", 0.0) / flt(row.get("qty", 0), self.float_precision), self.float_precision)
		# 			row["base_rate"] = flt(row.get("base_amount", 0.0) / flt(row.get("qty", 0), self.float_precision), self.float_precision)
		# 			# row.base_rate = flt(row.base_amount / flt(row.qty), self.float_precision)
		# 			row["basic_rate"] = flt(row.get("basic_rate", 0.0), self.float_precision)
		# 		else:
		# 			if self.is_not_invoice_row(row):
		# 				row.buying_rate, row.base_rate, row.basic_rate = 0.0, 0.0, 0.0

		for item_code, rows in self.grouped.items():
			for row in rows:
				row["gross_profit"] = row.get("base_amount", 0.0) - row.get("buying_amount", 0.0)
				row["gross_profit_percent"] = (
					(row["gross_profit"] / row["base_amount"]) * 100.0
					if row.get("base_amount", 0.0)
					else 0.0
				)
			...
			if self.filters.get("group_by") == "Invoice":
				for row in self.grouped[key]:
					...
					if "base_amount" not in row:
						row["base_amount"] = 0.0
					if row.get("indent") == 1.0:
						if (
							row.get("parent") in self.returned_invoices
							and row.get("item_code") in self.returned_invoices[row.get("parent")]
						):
							returned_item_rows = self.returned_invoices[row.get("parent")][row.get("item_code")]
							for returned_item_row in returned_item_rows:
								if "buying_amount" not in returned_item_row:
									returned_item_row["buying_amount"] = 0.0
								if "base_amount" not in returned_item_row:
									returned_item_row["base_amount"] = 0.0
				...
				for i, row in enumerate(self.grouped[key]):
					...
					if row.get("is_return"):
						invoice_portion = 100
					elif row.get("invoice_portion"):
						invoice_portion = row.invoice_portion
					elif row.get("payment_amount"):
				...
				for i, row in enumerate(self.grouped[key]):
					...
					if i == 0:
						...
						# if "buying_amount" not in row:
						# 	row["buying_amount"] = 0.0
						new_row["buying_amount"] += flt(row.get("buying_amount", 0), self.currency_precision)
						new_row["base_amount"] += flt(row.get("base_amount", 0), self.currency_precision)
	...
	def set_average_rate(self, new_row):
		if "buying_rate" not in new_row:
			new_row["buying_rate"] = 0.0
		if "base_rate" not in new_row:
			new_row["base_rate"] = 0.0
		self.set_average_gross_profit(new_row)
		new_row["buying_rate"] = (
			flt(new_row["buying_rate"] / new_row["qty"], self.float_precision) if new_row["qty"] else 0
		)
	...
	def get_buying_amount(self, row, item_code):
		...
		if item_code in self.non_stock_items and (row.project or row.cost_center):
			...
			# if not my_sle:
				# print(f"No Stock Ledger Entries for {item_code} in {warehouse}")
			if (row.update_stock or row.dn_detail) and my_sle:
				...
				# print(f"Using Delivery Note for {item_code}")
				...
				# print(f"Using Sales Order for {item_code}")
				incoming_amount = self.get_buying_amount_from_so_dn(row.sales_order, row.so_detail, item_code)
				if incoming_amount:
					return flt(row.qty) * incoming_amount
			else:
				# print(f"Falling back to Average Buying Rate for {item_code}")
				return flt(row.qty) * self.get_average_buying_rate(row, item_code)
	...
	def get_average_buying_rate(self, row, item_code):
		...
		if item_code not in self.average_buying_rate:
			...
			# print(f"get_incoming_rate Args for {item_code}: {args}")

			average_buying_rate = get_incoming_rate(args)
			# if not average_buying_rate:
			# 	print(f"No Valuation Rate found for {item_code} using {args}")
	...
	def load_stock_entries(self):
		...
		self.stock_entries = query.run(as_dict=True)
		frappe.logger().debug(self.stock_entries)
